	...
	<%-- ---------- fnInsertMember() ---------- --%>	
		function fnInsertMember(){
			$('#insert_btn').on('click', function(){
				if( $('#no').val().length != 6 || isNaN($('#no').val()) ) {	// isNaN 사용자의 영문자 입력을 막기 위해 사용함 
				...
				$.ajax({
				   ...
				   dataType: 'json',
					...
					error: function(xhr){
						switch (xhr.status) {
						case 2001: // NumberFormatException "나이는 정수만 입력 가능합니다."
						case 2002: // AgeRangeException	    "나이는 0~100 사이만 입력 가능합니다."
						case 2003: // DuplicateException 	"동일한 회원 번호는 입력할 수 없습니다."
						case 2004: // NullPointerException	"value를 입력하세요."
						case 2005: // PersistenceException  "입력 데이터를 확인하세요"	: DB형식에 맞지않는 다른 예외
